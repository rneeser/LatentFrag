...
class InteractionsEvaluator(AbstractEvaluator):
    ...
    def __init__(self, plip_exec='plipcmd'):
...
class ChemblRingEvaluator(AbstractEvaluator):
    ...
    def __init__(self):
        self.ring_system_lookup = RingSystemLookup()  # ChEMBL
...
class FullEvaluator(AbstractEvaluator):
    def __init__(
            ...
            plip_exec: Optional[Union[Path, str]] = None,
        ...
        if plip_exec is not None:
            all_evaluators.append(InteractionsEvaluator(plip_exec=plip_exec))
        else:
